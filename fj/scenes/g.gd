...
var current_player: PColor = PColor.Red
signal player_switched(new_player: PColor)


func switch_player() -> void:
	current_player = PColor.Blue if current_player == PColor.Red else PColor.Red
	player_switched.emit(current_player)
	print("Switched to player: ", "RED" if current_player == PColor.Red else "BLUE")
...
func get_opponent(player: PColor) -> PColor:
	return PColor.Blue if player == PColor.Red else PColor.Red
