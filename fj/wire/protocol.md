...
| `must_select`  | int \| absent               | Optional, default `1`. Exactly this many options must be selected. The field is **omitted** when `must_select == 1` (single-select, the legacy contract). When present, it is always `>= 2`. Duplicates among the selected options are permitted iff the same wire-equal option appears multiple times in `options`. |
...
**Multi-select** (when `must_select >= 2`):
...
| Field     | Type           | Notes                                                                                                                                                                 |
| --------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `options` | array of object | Exactly `must_select` entries. Each must be structurally equal to one of the option dicts in the most recent `prompt` for this player. Order is preserved end-to-end. |
